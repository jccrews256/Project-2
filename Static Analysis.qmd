...
play_data2<-play_data2 |>
  mutate(play_time=(3600-game_seconds_remaining)/60)

ggplot(data=play_data2 |> filter(game_id==2018090600),aes(x=play_time,y=home_wp,color=home_team))+geom_line(linewidth=1.5,na.rm=TRUE)+scale_color_nfl(type = "primary") + scale_y_continuous(limits=c(0,1))+
  labs(title="Win Probability for the Eagles in their Week 1 Game Against the Falcons",x="Game Time",y="Eagles Win Percentage")
```
